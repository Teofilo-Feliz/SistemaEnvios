...
export const useAuthStore = defineStore('auth', () => {
  ...
  const affiliateId = computed(() => {
    const id = Number.parseInt(String(affiliate.value ?? '').split(',', 1)[0].trim(), 10)
  ...
  const hasRole = computed(() => (required=[]) => required.some(role => roles.value.includes(role)))
  const hasAssignedRole = computed(() => (required=[]) => required.some(role => assignedRoles.value.includes(role)))
  const hasPermission = computed(() => (required=[]) => required.some(permission => permissions.value.includes(permission)))
  ...
  async function renewSilent() { if (!userManager) return null; try { const result = await userManager.signinSilent(); if (result) setIdentity(result); return result } catch { return null } }
  ...
  async function checkSession() {
    if (!userManager) { logoutLocal(); return false }
    const current = await userManager.getUser().catch(() => null)
    if (current && !current.expired) { setIdentity(current); return true }
    const renewed = current ? await renewSilent() : null
    if (!renewed) logoutLocal()
    return Boolean(renewed)
  }
  async function getValidToken() { if (user.value?.access_token && !user.value.expired) return user.value.access_token; const result = await renewSilent(); return result?.access_token || null }
  ...
  async function logout() { if (!userManager) return logoutLocal(); try { await authService.signoutRedirect() } catch { logoutLocal() } }
  function forceLogoutAndRedirectToLogin(returnUrl) { return userManager ? authService.signinRedirect(returnUrl ? { state: { returnUrl } } : undefined) : null }
  ...
  function handleUnauthorized() {
    ...
    unauthorizedInFlight = (async () => {
      ...
      logoutLocal()
      await forceLogoutAndRedirectToLogin(location.pathname + location.search)
    })().finally(() => { unauthorizedInFlight = null })
