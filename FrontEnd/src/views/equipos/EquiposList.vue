...
const rows = computed(() =>
  equipment.value.map((x) => ({
    ...x,
    id: x.equipoId,
    type: types.value.find((t) => t.tipoEquipoId === x.tipoEquipoId)?.nombre || `Tipo #${x.tipoEquipoId}`,
    brandModel: `${x.marca} ${x.modelo}`,
    serial: x.numeroSerie || '—',
    asset: x.codigoActivo || '—',
    location: locations.value.find((l) => l.ubicacionId === x.ubicacionActualId)?.nombre || `Ubicación #${x.ubicacionActualId}`,
  })),
)
...
function parametros() {
  ...
  for (const rule of filters.value.rules || []) {
    ...
    else if (['numeroSerie', 'codigoActivo', 'marca', 'modelo'].includes(rule.field)) textos.push(rule.value)
  }
...
<template>
  <div>
    <PageHeader title="Equipos" subtitle="Equipos registrados y asociados a envíos">
      <RouterLink class="btn btn-primary" to="/equipos/nuevo"><Plus :size="16" /> Nuevo equipo</RouterLink>
      <button class="btn btn-ghost" :disabled="loading" @click="load"><RefreshCw :size="16" /> Actualizar</button>
    </PageHeader>
    <AdvancedFilter :fields="equipmentFilterFields" :sources="sources" :loading="loading" @search="search" @clear="clear" />
    ...
    >
      ...
      <template #footer>
        <button class="btn btn-ghost" type="button" @click="router.push(`/equipos/${selected.equipoId}`)">
