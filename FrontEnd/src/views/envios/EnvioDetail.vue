...
import { ArrowLeft, MapPin, Package, Pencil, Printer, RefreshCw } from "lucide-vue-next";
...
const locationName = (id) =>
  locations.value.find((x) => x.ubicacionId === id)?.nombre || `Ubicación #${id}`;
const stateCode = (id) => states.value.find((x) => x.estadoEnvioId === id)?.codigo || "";
const stateName = (id) => states.value.find((x) => x.estadoEnvioId === id)?.nombre || stateCode(id);
const esEnvioHaciaFilial = computed(() => {
  const direccion = shipment.value?.direccion;
  return (
    direccion === 2 ||
    String(direccion).toLowerCase() === "haciafilial" ||
    String(direccion).toLowerCase().includes("filial")
  );
});
const flowLabel = computed(() =>
  esEnvioHaciaFilial.value ? "Tecnología → Filial" : "Filial → Tecnología",
);
...
async function load() {
  ...
  try {
    ...
    ] = await Promise.all(requests.map((request) => request.catch(() => ({ data: null }))));
    ...
    ui.showToast(error.userMessage || "No fue posible cargar el envío.", "error");
  } finally {
    loading.value = false;
  }
}
async function confirmTransport() {
  if (
    ...
    stateCode(shipment.value?.estadoEnvioId) !== "ENTREGADO_TRANSPORTACION" ||
    transport.value.entregaConfirmada
  )
    return;
  if (
    !(await confirmAction({
      title: "Confirmar recepción en Transportación",
      text: `¿Confirmas la recepción del envío ${shipment.value.numeroEnvio}? Pasará a En tránsito.`,
  ...
  try {
    ...
    ui.showToast(error.userMessage || "No fue posible confirmar la transportación.", "error");
  } finally {
...
async function deliverToTransport() {
  ...
  try {
    ...
    ui.showToast(error.userMessage || "No fue posible registrar la entrega.", "error");
  } finally {
...
async function deliverTechnologyToTransport() {
  if (
    !(await confirmAction({
      title: "Entregar a transportación",
      text: `¿Confirmas que el envío ${shipment.value.numeroEnvio} fue entregado a Transportación? Quedará esperando que le asignen chofer.`,
      confirmText: "Entregado a transportación",
    }))
  )
    return;
  delivering.value = true;
  try {
    await envioService.dispatchFromTechnology(route.params.id);
    ui.showToast("Entrega registrada. Transportación debe asignarle chofer.", "success");
    await load();
  } catch (error) {
    ui.showToast(error.userMessage || "No fue posible registrar la entrega.", "error");
  } finally {
...
async function registerTechnologyArrival() {
  ...
  const accepted = await confirmAction({
    title: interno ? "Confirmar llegada a Transportación" : "Confirmar recepción privada",
    text: `¿Deseas confirmar la llegada del envío ${shipment.value.numeroEnvio}?`,
  ...
  try {
    await (interno
      ? envioService.confirmTransportArrival(route.params.id)
      : envioService.registerTechnologyArrival(route.params.id));
    if (interno) notifyNotificationsChanged();
    ui.showToast(
      interno ? "Llegada confirmada. Tecnología fue notificada." : "Recepción privada registrada.",
      "success",
    );
    await load();
  } catch (error) {
    ...
    registeringArrival.value = false;
  }
}
function printPage() {
  window.print();
...
<template>
  ...
  <button
    v-if="
      ...
      esEnvioHaciaFilial &&
      auth.can('envios.despachar') &&
      stateCode(shipment.estadoEnvioId) === 'PREPARACION_TECNOLOGIA'
    "
    class="btn btn-primary shipment-delivery-action"
    :disabled="delivering"
    type="button"
    @click="deliverTechnologyToTransport"
  >
  ...
  <div v-else-if="shipment">
    <PageHeader
      ...
      ><button
        v-if="esEditable && auth.can('envios.editar')"
        class="btn btn-primary"
        @click="router.push(`/envios/${shipment.envioId}/editar`)"
      >
        <Pencil :size="16" /> Editar envío</button
      ><button class="btn btn-secondary" @click="printPage"><Printer :size="16" /> Imprimir</button
    ...
    <BaseCard class="shipment-hero"
      ...
      <div class="shipment-facts">
        <div>
          <MapPin :size="18" /><span
            ><small>Origen</small
            ><strong>{{ locationName(shipment.ubicacionOrigenId) }}</strong></span
          >
        </div>
        <div>
          <MapPin :size="18" /><span
            ><small>Destino</small
            ><strong>{{ locationName(shipment.ubicacionDestinoId) }}</strong></span
          >
        </div>
        <div>
          <span
        ...
        <div>
          <Package :size="18" /><span
            ><small>Cantidad equipos</small><strong>{{ equipment.length }}</strong></span
          >
        </div>
      </div></BaseCard
    >
    ...
    <div v-if="tab === 'summary'" class="detail-grid">
      <BaseCard title="Progreso del envío"
        ><ShipmentTimeline
          :events="events"
          @ver-incidencia="incidenciasAbiertas = true" /></BaseCard
    ...
    <BaseCard v-else-if="tab === 'equipment'" title="Equipos del envío" :padded="false"
      ...
      ><div v-if="transport">
        ...
        <button
          v-if="
            ...
            stateCode(shipment.estadoEnvioId) === 'ENTREGADO_TRANSPORTACION' &&
    ...
    <BaseCard v-else-if="tab === 'reception'" title="Recepción"
      ...
      <div v-else class="empty-state">No hay registro de recepción.</div></BaseCard
    >
    <BaseCard v-else title="Incidencias"
      ><div v-if="incidenciasRecepcion.length" class="incidencias-recepcion">
        ...
        <article v-for="item in incidenciasRecepcion" :key="item.envioEquipoId">
          ...
          <p class="anotacion">
            {{ item.observaciones || "Se marcó con incidencia sin detallar." }}
          </p>
  ...
  <div v-if="deliveryOpen" class="confirm-overlay" role="dialog" aria-modal="true">
    <div class="confirm-dialog">
      <h2>
        {{
          isPrivateTransport(transport?.estrategia)
            ? "Entregar a transporte privado"
            : "Entregar a transportación"
        }}
      </h2>
      <p>
        {{
      ...
      <div class="confirm-actions">
        <button class="btn btn-secondary" type="button" @click="deliveryOpen = false">
          Cancelar</button
        ><button
          class="btn btn-primary"
          type="button"
